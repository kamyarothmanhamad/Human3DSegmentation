...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, c=6, k=13):
        ...
        #stride, nsample = [1, 4, 4, 4, 4], [32, 64, 64, 64, 64]
    ...
    def forward(self, pxo):
        ...
        num_points = p0.shape[0]//batch_size
        ...
        x = self.cls(x1)
        #x = x.reshape(batch_size, num_points, self.k)
        return x
...
def from_cfg(cfg: dict):
    c, k = cfg["c"], cfg["k"]
    model = PointTransformerSeg(PointTransformerBlock, [2, 3, 4, 6, 3], c=c, k=k)
    return model


if __name__ == "__main__":
    model = pointtransformer_seg_repro(c=3).to(1)
    B = 2
    N_per_batch = 10000
    N = B * N_per_batch

    p = torch.rand(N, 3).to(1)
    x = torch.rand(N, 3).to(1)
    o = torch.tensor([(i + 1) * N_per_batch for i in range(B)], dtype=torch.int32).to(1)

    out = model((p, x, o))
    print(out.shape)  # Should be (N, num_classes)